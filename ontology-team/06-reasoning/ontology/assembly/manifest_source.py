...
_E = [
    ...
    ("ontology-metadata", "kosha-ontology-metadata.ttl", "metadata", "release",
     {"SRV", "CON"}, "turtle",
     "A4: owl:Ontology 헤더 + owl:versionIRI 1.0.0 + dcterms/VoID 공개 메타데이터. annotation-only(DL 영향 0). namespace는 A2(영속 IRI)에서 w3id로 마이그레이션."),
    # ── 생성 산출물: 추론 diff TTL (PG sr_inferred_relations 물질화 입력; reasoning profile 아님, keep-loose) ──
    ("inferred-r1-r2", "kosha-inferred-relations.ttl", "generated-inferred", "materialization",
     set(), "turtle", "R-1 exemptedBy/R-2 coApplicable diff. 생성: scripts/emit_inferred_relations.py --mode strict. PG 물질화 입력(import_sr_inferred_relations_to_pg.py)."),
    ("inferred-kr2", "kosha-coapplicable-chapter.ttl", "generated-inferred", "materialization",
     set(), "turtle", "K-R2 같은 Chapter coApplicable. 생성: emit_inferred_relations.py --mode chapter. PG 물질화 입력."),
    ("inferred-kr4", "kosha-dependson-hazard.ttl", "generated-inferred", "materialization",
     set(), "turtle", "K-R4 같은 Hazard dependsOn. 생성: emit_inferred_relations.py --mode hazard. PG 물질화 입력."),
    ("codes-skos", "kosha-codes-skos.ttl", "generated-skos", "vocabulary",
     set(), "turtle", "A5: 위험 코드 SKOS ConceptScheme(공개 발행물). 생성: scripts/gen_skos_scheme.py. reasoning 미편입(skos↔owl punning 회피)."),
    ...
    ("facet-l2tune", "kosha-facet-l2tune-patch.ttl", "tbox-patch", "L3-reasoning",
     {"SRV", "CON", "MAT", "FAC"}, "turtle",
     "OWA→CWA L2 보강 TBox: agent:OxygenDeficiency(PG가 hazardous_agent 축으로 쓰는 산소결핍; haz:OxygenDeficiency 사고축과 병존). hand-authored patch."),
    ("facet-state", "kosha-facet-state-patch.ttl", "tbox-patch", "L3-reasoning",
     {"SRV", "CON", "MAT", "FAC"}, "turtle",
     "CAT-4 Stage2: 비주축 차원(ppe/env/activity/agent_state) 구체값 39 NamedIndividual(부모 ctx:PPEState 등). 전량 SHE export로 표면화. 생성: data-team/04-ontology-export/export-scripts/gen_facet_state_patch.py."),
    ...
    ("facet-axis-disjoint", "kosha-facet-axis-disjoint.ttl", "axioms-disjoint", "L3-reasoning",
     {"SRV", "CON", "MAT", "FAC"}, "turtle", "facet 9축 상호 disjoint (B3a/F2). risk:RiskFeature children (haz:Hazard는 B5/F4-c로 폐지)."),
    ...
    ("abox-she-full", "kosha-instances-she-full.ttl", "abox-derived", "L3-reasoning",
     {"SRV", "CON", "MAT", "FAC"}, "turtle",
     "SHE 패턴 ABox — PG she_catalog 서빙 전량(1675, status approved_auto/manual). SSOT=PG (CAT-4). 생성: data-team/04-ontology-export/export-scripts/export_she_catalog_to_abox.py --scope active. legacy 965(abox-she)·l2tune 37을 대체(diff 손실 0, full ⊇ 둘 다). cross-axis 4종/3축 미해석 2종 feature는 비일관 방지로 미발행(카탈로그 정정 대기)."),
    # ↓ CAT-4: full(1675)이 둘을 superset 포함(diff 손실 0) → orphan-but-keep로 퇴역(서빙 미적재).
    ("abox-she", "kosha-instances-she.ttl", "abox-derived", "L3-reasoning",
     set(), "turtle",
     "[퇴역 2026-06-12, abox-she-full로 대체] legacy 965 SHE ABox(frozen she-instances-v1.ttl 유래). full ⊇ legacy(diff 0). orphan-but-keep(history)."),
    ("abox-she-l2tune", "kosha-instances-she-l2tune.ttl", "abox-derived", "L3-reasoning",
     set(), "turtle",
     "[퇴역 2026-06-12, abox-she-full에 포함] OWA→CWA L2 보강 37패턴 surgical export. full ⊇ l2tune. orphan-but-keep(history)."),
    ("abox-guide-fine", "kosha-instances-guide-fine.ttl", "abox-derived", "L3-reasoning",
     {"SRV", "CON", "MAT", "FAC"}, "turtle",
     "WC-A/B: LLM-enriched guide fine 태그 ABox (guide→fine accident/agent/wc, fine⊑canonical로 canonical 추론). 생성: scripts/gen_guide_fine_abox.py(PG GF method=llm_enriched_*). 서빙=PG, 온톨로지 정합용."),
    ...
    # ── ARCHIVE (어떤 소비자도 안 읽음; silent-orphan 방지 등록, P1.6b 물리이동 → archive/) ──
    ("arc-v3-subclass", "archive/kosha-ontology-v3-subclass-patch.ttl", "archive", "-",
     set(), "turtle", "구 catalog sub 기반 subclass-patch(haz/agent, casing 단절). facet-taxonomy로 대체 → archive(P3)."),
    ("arc-v1-annotated", "archive/kosha-ontology.annotated.ttl", "archive", "-",
     set(), "turtle", "v1 annotated snapshot(obsolete)."),
    ("arc-legacy-swrl1", "archive/kosha-rules.swrl", "archive", "-",
     set(), "turtle", "legacy SWRL monolith(미사용)."),
    ("arc-legacy-swrl2", "archive/kosha-rules-v2.swrl", "archive", "-",
     set(), "turtle", "legacy SWRL monolith v2(미사용)."),
    ("arc-old-inversion", "archive/kosha-instances-guide-hazard.ttl", "archive", "-",
     set(), "turtle", "구 PG-inversion ABox(폐기 예정)."),
    ("arc-serving-policy", "archive/serving-policy.ttl", "archive", "-",
     set(), "turtle", "serving policy 산출물(온톨로지 입력 아님)."),
    ("arc-shapes-v1", "archive/serving-validation-shapes.ttl", "archive", "-",

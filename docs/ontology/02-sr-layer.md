...
`ci_cross_guide_broad_only_guard1` 기준 OHS 서빙에서는 SR을 `direct SR`과 `broad secondary SR`로 구분한다. broad SR은 검색 확장과 보조 점수에는 사용할 수 있지만, 단독으로 top Guide/CI나 법적 확정 근거를 만들 수 없다.
...
- `app:servingRole "broad_secondary_only"`는 core SR ontology의 법적 의미가 아니라 `serving-snapshot-ci_cross_guide_broad_only_guard1.ttl`의 검증용 정책 주석이다. 현재 broad SR은 12개이며, 단독 top Guide/CI 생성 금지 규칙은 `serving-validation-shapes.ttl`과 `validate_serving_snapshot.py`에서 검사한다.

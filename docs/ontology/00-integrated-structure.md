...
- `ci_cross_guide_broad_only_guard1` 기준 Guide/CI 서빙 정책은 OHS JSON/PG에서 실행하고, 별도 `serving-snapshot-*.ttl`로 export해 SHACL/SPARQL 검증에 사용한다. 직전 기준 `ci_unrelated_action_filter1`은 unrelated generic CI 차단 비교 기준으로 남기고, `ci_preferred_guide_ci1`은 top-Guide CI 정렬 비교 기준으로 남기며, `ci_candidate_promotion_v1`은 CI 후보 승격 비교 기준으로 남긴다.
...
flowchart TB
    ...
    subgraph GUIDE_POLICY["5B. Guide Serving Policy / Validation Snapshot"]
      direction TB
      GuideUsageProfile["가이드 사용 프로필<br/>(guide:GuideUsageProfile)<br/>검증용 스냅샷"]
      ...
      PrimaryWorkProcess["우선 작업절차<br/>guide:primaryWorkProcess"]
      ServingSnapshot["서빙 스냅샷 TTL<br/>serving-snapshot-ci_cross_guide_broad_only_guard1.ttl"]
      ValidationReport["검증 리포트<br/>hard violation / warning"]
    end
    ...
    KoshaGuide -->|"guide:hasUsageProfile<br/>validation snapshot"| GuideUsageProfile
    GuideUsageProfile -->|"guide:photoMatchability"| PhotoMatchability
    GuideUsageProfile -->|"guide:procedureRole"| ProcedureRole
    GuideUsageProfile -->|"guide:negativeBoundaryTerm"| NegativeBoundary
    GuideUsageProfile -->|"guide:primaryWorkProcess"| PrimaryWorkProcess
    PrimaryWorkProcess -.->|"points to"| WorkProcess
    GuideUsageProfile -.->|"exported into"| ServingSnapshot
    ServingSnapshot -.->|"SHACL/SPARQL validation"| ValidationReport
    ...
    subgraph RUNTIME["서빙용 물질화 함수와 저장소"]
      ...
      ServingArtifacts["OHS serving artifacts<br/>guide_domain_profiles / photo_matchability / broad_sr_policy"]
      ExportServingSnapshot["export_serving_snapshot.py<br/>JSON/report → validation TTL"]
      ValidateServingSnapshot["validate_serving_snapshot.py<br/>SPARQL validation"]
    ...
    GuideTables -.->|"runtime Guide/WP/CI 조회"| ServingArtifacts
    ServingArtifacts -.->|"validation export"| ExportServingSnapshot
    ExportServingSnapshot -.->|"serving-policy.ttl / serving-snapshot.ttl"| ServingSnapshot
    ValidateServingSnapshot -.->|"hard violation 0 / warnings"| ValidationReport
    ...
    subgraph IMPROVE["9. 온톨로지 개선 루프"]
      ...
      Review["예외 검토"]
      Approved["승인된 패턴 사전"]
      AnomalyQueue["스냅샷 이상치 큐<br/>structure / data / algorithm / corpus gap"]
    end

    RawLog -.->|"운영 로그 분석"| Candidate
    ResultScreen -.->|"사용자/검토자 피드백"| Candidate
    ValidationReport -.->|"기계 검증 이상치"| AnomalyQueue
    AnomalyQueue -.->|"원천 JSON/PG/export script 수정"| Candidate
    Candidate --> Validator
    ...
    class KoshaGuide,ChecklistItem,WorkProcess,DomainTerm,EquipmentSpec,DocumentRequirement,GuideInterLink,GuideUsageProfile,PhotoMatchability,ProcedureRole,NegativeBoundary,PrimaryWorkProcess guide;
    ...
    class NormalizeFeatures,ApplyRules,MatchShe,ClassifyStatus,QuerySR,GetChecklist,GetGuides,BuildActions,SheCatalog,SheSRMap,SheCIMap,GuideTables,ServingArtifacts,ExportServingSnapshot,ValidateServingSnapshot,ServingSnapshot,ValidationReport,BootstrapShe,ExportShe runtime;
...
`GuideUsageProfile`과 `serving-snapshot-ci_cross_guide_broad_only_guard1.ttl`은 런타임 의사결정용 원천이 아니라 검증용 스냅샷이다. 런타임은 계속 PostgreSQL과 OHS JSON artifact를 읽고, 스냅샷은 `photo_unmatchable` top 추천, broad SR 단독 추천, `needs_review` 후보 사용 같은 이상한 조합을 기계적으로 찾는 데 쓴다.
...
| 상세도 | 통합도 반영 위치 |
|---|---|
| [`01-law-layer.md`](01-law-layer.md) | `4A. 법령 레이어` |
| [`02-sr-layer.md`](02-sr-layer.md) | `4B. 안전요구사항 레이어` |
| [`03-risk-situation-layer.md`](03-risk-situation-layer.md) | `2-3. 위험 지식 공통 계층과 SHE 패턴`, `서빙용 물질화 함수와 저장소` |
| [`04-guide-layer.md`](04-guide-layer.md) | `5. KOSHA Guide 레이어`, `서빙용 물질화 함수와 저장소` |
| [`05-penalty-layer.md`](05-penalty-layer.md) | `6. 벌칙 레이어`, `7-8. 판정, 조치, 사업주 안내` |
...
core Guide A-Box: 1,038 Guide / 9,316 WorkProcess / 54,631 ChecklistItem
primary_workprocess_not_in_base_ttl: 1,220 -> 0
guide_usage_profiles PG sync: PASS, 1,038 rows, missing Guide 0, missing primary WP 0, cross-guide primary WP 0
```

남은 핵심 작업은 status-level risk inference를 넓히는 것이 아니라, validation report와 Stage 5 큐를 보면서 `CI no_action 495`, `CI guide_boundary_mismatch 1`, `workprocess_mismatch 5`, `NO_TOP 88`을 WorkProcess/CI relevance와 Guide usage profile로 구조적으로 보강하는 것이다. 단, `NO_TOP`은 무조건 실패가 아니며 사진 현장에 맞는 KOSHA Guide가 없을 때는 표준절차를 비워 두는 것이 맞다.

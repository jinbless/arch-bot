...
- **recall 상향 블록 ✅전부 완료**(EVAL-1 `guide_coverage_rate` FN veto 위에서 안전): GATE-2(`c72e89a`) · GATE-3(`cd0ec9f`) · DEEP-1(`2496082`) 모두 full replay regression PASS(v4 baseline, gated 12지표 delta 0.0000). **다음 후보 = (a)** DEEP-1 frontend 렌더(source_path/review_required 배지 — 현재 backend-only, API엔 노출되나 프론트 미렌더) **(b)** DRIFT/OBS/PROV 서빙측 측정 항목 **(c)** DEEP-2(BFO 축, 온톨로지측 독립) **(d)** Phase 2 gold(EVAL-2, 사용자 D14). 검증은 `make f1-regression`(v4 baseline) + judge eval(precision).

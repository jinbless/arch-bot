...
| F5 | 빈 축 → **해결(SHE 통합)** | ctx 5 sub축은 개체 보유+she:has* range 타깃이나 used=0이었음 — 진짜 원인은 **SHE ABox(965패턴×6축)가 데이터팀에만 있고 온톨로지 미통합 + legacy 어휘**. **she-instances를 canonical forward 마이그레이션해 온톨로지 편입**(`kosha-instances-she.ttl`, gen_she_abox.py) → she:hasPPEState 등 **0→965, 5축 실채움**. triggerText datatype range도 완화. ⚠️**2026-06-12 CAT-4**: abox-she-full **1,675** 단일화·965 퇴역 | 中 | ✅(SHE통합) |

...
| F5 | 빈 축 → **재특성화** | ctx 5 sub축은 subclass 0이나 **개체 보유**(AgentState 5/EnvFactor 10/PPEState 17/Temporal 6/WorkActivity 11) + she:has* range 타깃 → **구조 결함 아님**. she:has*가 코퍼스 used=0(SHE는 flat WorkContext) = **SHE/data 모집단 갭**(ontology 무관). 비-canonical 개체 prune은 fine-vocab 정책(별도) | 低/data | 조사완료 |

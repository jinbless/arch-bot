...
b. ~~legacy 갈래 계산 옵션화~~ **완료(2026-08-09, 커밋 33dd14f·prod 배포)** — 조문후보·벌칙
   3경로·표준절차·근거추적(+trace 전용 article_ids 조회)을 기본 제외, `OHS_LEGACY_EXTRAS=1`로 복원.
   ★ 유지된 것과 이유: hazard_guide_relations(③자료 카드 소비)·checklist_rows(흐름 없을 때 CI
   폴백)·SHE 매칭(finding_status 입력)·findings(_summary 폴백). 이 넷을 끊으면 화면이 깨진다.
   레이턴시 지배 요인은 여전히 Vision(42~146s 변동) — 추가 단축은 Vision 쪽 과제.

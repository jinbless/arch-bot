...
| 지표 | 값 | CI95 | 비고 |
|---|---|---|---|
| 관 단위 정확 일치(exact) | **0.784** | [0.667, 0.882] | 키 정규화 후. 경로: 0.647 → 상속 0.647 → 판별 단서 0.725 → Vision v2 0.765 → **키 정규화 0.784** |
| **흐름 기준 유효율 (신설)** | **0.941** | [0.863, 1.000] | exact ∪ 선택 그룹 **흐름**이 gold 조문 포함. 0.647 → 0.824 → 0.902 → 0.922 → **0.941** |

...
| 지표 | 값 | CI95 | 비고 |
|---|---|---|---|
| 관 단위 정확 일치(exact) | **0.765** | [0.647, 0.882] | Vision v2 채택 후. 경로: 0.647 → 상속 0.647 → 판별 단서 0.725 → **Vision v2 0.765** |
| **흐름 기준 유효율 (신설)** | **0.922** | [0.843, 0.980] | exact ∪ 선택 그룹 **흐름**이 gold 조문 포함. 0.647 → 0.824 → 0.902 → **0.922** |
...
백업 기준으로만** 유효하다. 잔여 오인식 12장 + 예측 빈 사진 1장.

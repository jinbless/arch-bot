...
## 앵커 — 추락·붕괴(절1·절2) 카탈로그 편입 (2026-08-13 ⭐ 현행)

카탈로그 106종(장6 카탈로그 복귀)·채점 97장 기준. 개구부·단부·계단·지붕 gold가 처음 채점
가능해져 모수가 51→97장으로 늘었다 — **아래 2026-08-10 절(51장)과 집계 직접 비교 불가**,
연속성은 per-photo 게이트(G1 히트→미스 0)로 보장. 정본 dev-note:
[fall-flow-catalog-2026-08-13](../dev-notes/fall-flow-catalog-2026-08-13.md).

| 지표 | 값 | CI95 | 비고 |
|---|---|---|---|
| 관 단위 정확 일치(exact) | **0.959** | [0.918, 0.990] | 기존 51장 히트→미스 0 + 신규 46장 전부 히트 |
| 흐름 기준 유효율 | **0.990** | [0.969, 1.000] | 완전 오인식 4장(4.1%) · 예측 빈 사진 0장 |

게이트(사전등록, `backend/scripts/measure_fall_catalog_gates.py`): G1 기존 채점 히트→미스 0 ·
G-FIRST 주 앵커(첫 키) gold 밖 절도 0(변경 10장 전부 gold 안 — exact가 집합 교집합이라 이
채널은 별도 게이트가 필요) · G2 정답 밀림 0. prod 실증: 경북환경 계단 사진 재분석 —
앵커 절1 추락(kind=장소)·'지금 당장' 6건·제43조 개구부 방호가 AI 대조 matched로 최상단.

## 앵커 — 흐름 기준 유효율 신설 + 비계 공통 상속(A안) (2026-08-10, 카탈로그 104종·51장 기준)
...
| 지표 | 값 | CI95 | 비고 |
|---|---|---|---|
| 관 단위 정확 일치(exact) | **0.784** | [0.667, 0.882] | 경로: 0.647 → 상속 0.647 → 판별 단서 0.725 → Vision v2 0.765 → 키 정규화 **0.784** → miss11 레버 0.784(노원 +1·한솔2 -1 상쇄) |
| **흐름 기준 유효율 (신설)** | **0.961** | [0.902, 1.000] | exact ∪ 선택 그룹 **흐름**이 gold 조문 포함. 0.647 → 0.824 → 0.902 → 0.922 → 0.941 → **0.961** |

**Vision v2 관찰 체크리스트**(같은 날 3차): 잔여 오인식의 원인이 Vision 서술 누락(소화기·분전반·
접지·밀폐가 사진에 있는데 서술이 안 담음)으로 좁혀져, 소방·전기·밀폐·환기·대형설비 부위·방호덮개
6계통을 "보이면 반드시 서술(부재 추정 금지)"로 프롬프트에 추가. A/B(51장): exact 0.725→0.784·악화 0
→ 채택, gold Vision 캐시 129장 재판독(`regen_vision_gold.py`, v1은 .v1.json 백업) + RESOLVE 재실행.
서빙 IMAGE_ANALYSIS_PROMPT에도 동일 이식. 캐시 무효화에 Vision 내용 해시 추가(프롬프트 변경 자동 감지).
⚠ gold Vision 캐시가 v2로 바뀌었으므로 **이전 캐시 기반 측정(cue-pool 천장 등)과의 직접 비교는 v1
백업 기준으로만** 유효하다.

**group_key 카탈로그 정규화**(같은 날 4차, LLM 재호출 0): terra 실험
([dev-note](../dev-notes/terra-opus-experiment-2026-08-10.md)) 부산물로 gpt-5.4가 카탈로그에 없는
변형 키(`절8 사출성형기 등 > 관8?` — 하위 계층 지어붙임)를 내면 채점·서빙이 조용히 버리는 것을
발견(겉보기-오류=버그 클래스 9번째). 카탈로그 키가 앞부분과 일치(경계 공백/`>`)하면 가장 긴 키로
정규화하는 규칙을 측정(`measure_anchor_accuracy.norm_gk`)과 서빙(`cue_article_service._norm_gk`)에
동일 적용 → exact 0.765→**0.784** · 흐름 유효율 0.922→**0.941**. 잔여 오인식 11장 + 예측 빈 사진 0장.
같은 실험에서 모델 전환(gpt-5.6-terra Vision+RESOLVE)은 이득 없음으로 비채택(exact 0.745·flow 0.922),
Claude Opus 5 블라인드 검수는 gold 판가름 3장 전부 일치.

**miss11 분해 후속 레버 ①②③**(같은 날 5차 — [miss11-decompose](../dev-notes/miss11-decompose-2026-08-10.md)):
Vision 체크리스트 v3(흡연 흔적 세목) + 카탈로그 판별단서 3종(화기 절2 흡연 세목 / 보건편 '절2 설비기준
등' 실험실ㆍ배기 후드 / 절5 말비계ㆍ이동식비계 형태 단서). gold Vision 129장 v3 재판독(v2는 .v2.json 백업)
+ RESOLVE 재실행. **flow 0.941→0.961(악화 0)** — 노원병원 검사실이 exact·flow 회복(레버 ② 성공).
exact는 0.784 유지: 한솔 '현장사진 2'가 Vision **재판독 변동**(v2 '이동식 비계'→v3 '고소 작업대' 명명)으로
절5→절3 이동해 -1(흐름은 상속으로 유지 — 화면 피해 없음), 레버 ③ 라벨로는 미회복(장면 서술에 훅 없음).
레버 ①(흡연 흔적)은 v3 지시에도 gpt-4.1이 소형 물체(재떨이 깡통·라이터)를 여전히 미서술 — **4.1 지각
한계**로 판정(Opus/Fable은 봄). ⚠ 이름 매칭 함정 3번째 실측: '국소배기장치' 라벨 토큰이 법 제42조
유해위험방지계획서 부착을 절3에서 빼앗음 → '배기 후드'로 우회(flow diff 0 확인). ⚠ gold Vision 재생성은
**전 사진 재추첨**이다 — 한 장을 고치려 129장을 다시 읽으면 다른 경계 사진이 뒤집힐 수 있다(한솔2가 그 사례).

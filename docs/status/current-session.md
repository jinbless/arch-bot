...
- ✅ **B5/F4** (이 커밋): `haz:AccidentType ⊑ haz:Hazard` (신규 `kosha-hazard-axis-taxonomy.ttl`, 1 triple) — haz:Hazard **빈 축을 상위 범주로 재정의**. 근거: 4 Hazard-range 속성 객체가 코퍼스 **100% AccidentType**(738/2484/8/8, agent/ctx 0)=coextensive. **equivalentClass 대신 subClassOf**(약한 주장·향후 비-사고 hazard 여지, 사용자 결정). 삭제는 range/제약3/SWRL 파괴라 제외. 게이트: CON +1·신규, check_disjoint 0, 재적재 **owl:Nothing 200 count 0** + 추론 `haz:Fall⊑haz:Hazard`=true(catalog haz:Hazard ⊒1). F5(ctx 5 빈 sub축) 잔여.

**남은 batch** (findings 문서 B3b/B5-F5/B6): B3b 축내 disjoint(저가치, facet=property-linked라 ABox 충돌 못잡음) / **B5 잔여 F5**(ctx 5 빈 sub축: AgentState/EnvFactor/PPEState/TemporalStage/WorkActivity) / B6 BFO grounding(risk:RiskFeature=Quality인데 자식 Object/Process 혼재 + haz:Hazard grounding 재검토, top·고위험). **권고: 모두 코퍼스-aware + 실추론쿼리(owl:Nothing) 게이트로.**

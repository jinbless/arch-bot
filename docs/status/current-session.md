...
- 🔍 **F5/중복label/속성중복 triage** (코드 변경 0): 정밀조사로 셋 다 단순 결함 아님 — **F5** ctx 5축은 개체 보유+she:range라 구조 결함 아님(she:has* used=0=SHE/data 갭), **중복label**(F19) 정당한 homonym, **속성중복**(F20) sr:addressesHazard~addressesAccidentType는 둘 다 활성(다른 룰 서브시스템)이라 통합=별도 refactor. 백로그 정밀화. 잔여 선택지(사용자 결정): 다축 ctx prune·sr 속성 통합.

...
2026-05-07 기준 `OHS`는 위 통합 흐름을 기준으로 product 코드를 정리하는 중이다.
...
analysis_pipeline.py
  관찰 결과 → RiskFeature → SHE → SR/Guide/CI/PenaltyPath 응답 조립
...
guide_recommendation_service.py
  CI 기반 즉시 조치와 Guide/WorkProcess 기반 표준 개선 절차 구성
...
남은 핵심 작업은 `SHE/SR 후보를 놓치지 않는 것`이 아니라, `확정 위험`과 `확인 필요 후보`의 표시 경계를 더 정교하게 만드는 것이다.

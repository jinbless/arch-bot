...
- `corpus_gap_guard1` 이후 Guide 서빙 정책은 OHS JSON/PG에서 실행하고, 별도 `serving-snapshot-*.ttl`로 export해 SHACL/SPARQL 검증에 사용한다.
...
flowchart TB
    ...
    subgraph GUIDE_POLICY["5B. Guide Serving Policy / Validation Snapshot"]
      ...
      ServingSnapshot["서빙 스냅샷 TTL<br/>serving-snapshot-corpus_gap_guard1.ttl"]
...
`GuideUsageProfile`과 `serving-snapshot-corpus_gap_guard1.ttl`은 런타임 의사결정용 원천이 아니라 검증용 스냅샷이다. 런타임은 계속 PostgreSQL과 OHS JSON artifact를 읽고, 스냅샷은 `photo_unmatchable` top 추천, broad SR 단독 추천, `needs_review` 후보 사용 같은 이상한 조합을 기계적으로 찾는 데 쓴다.
...
```text
accepted baseline: corpus_gap_guard1
Python compile: OK
frontend npm run build: OK
synthetic Guide v1~v10 total: 2,360
current obvious top Guide mismatch: 22
NO_TOP: 85
...
serving snapshot validation: PASS, hard violation 0, warnings 16
core Guide A-Box: 1,038 Guide / 9,316 WorkProcess / 54,631 ChecklistItem
primary_workprocess_not_in_base_ttl: 1,220 -> 0
```

남은 핵심 작업은 status-level risk inference를 넓히는 것이 아니라, validation warning과 Stage 5 큐를 보면서 `CI no_action 482`, `CI guide_boundary_mismatch 26`, `workprocess_mismatch 20`, `NO_TOP 85`를 WorkProcess/CI relevance와 Guide usage profile로 구조적으로 보강하는 것이다.

...
- `ci_broad_sr_guard4` 기준 Guide/CI 서빙 정책은 OHS JSON/PG에서 실행하고, 별도 `serving-snapshot-*.ttl`로 export해 SHACL/SPARQL 검증에 사용한다. 직전 기준 `ci_wp_relevance_guard1`은 Guide/WorkProcess 경계 비교 기준으로 남기고, `no_top_serving_bridge4`는 status/penalty/SHE/SR 경계 비교 기준으로 남긴다.
...
flowchart TB
    ...
    subgraph GUIDE_POLICY["5B. Guide Serving Policy / Validation Snapshot"]
      ...
      ServingSnapshot["서빙 스냅샷 TTL<br/>serving-snapshot-ci_broad_sr_guard4.ttl"]
...
`GuideUsageProfile`과 `serving-snapshot-ci_broad_sr_guard4.ttl`은 런타임 의사결정용 원천이 아니라 검증용 스냅샷이다. 런타임은 계속 PostgreSQL과 OHS JSON artifact를 읽고, 스냅샷은 `photo_unmatchable` top 추천, broad SR 단독 추천, `needs_review` 후보 사용 같은 이상한 조합을 기계적으로 찾는 데 쓴다.
...
```text
accepted baseline: ci_broad_sr_guard4
Python compile: OK
...
남은 핵심 작업은 status-level risk inference를 넓히는 것이 아니라, validation report와 Stage 5 큐를 보면서 `CI no_action 492`, `CI guide_boundary_mismatch 22`, `workprocess_mismatch 5`, `NO_TOP 88`을 WorkProcess/CI relevance와 Guide usage profile로 구조적으로 보강하는 것이다. 단, `NO_TOP`은 무조건 실패가 아니며 사진 현장에 맞는 KOSHA Guide가 없을 때는 표준절차를 비워 두는 것이 맞다.

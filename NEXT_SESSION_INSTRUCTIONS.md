...
3. Guide 품질 작업은 `ci_unrelated_action_filter1` 기준으로 이어간다.
...
6. 다음 구조적 보강 대상은 `CI no_action 494`, `CI guide_boundary_mismatch 2`의 남은 꼬리다. `CI broad_sr_only`는 13건에서 0건으로 해소했다. 현장에 맞는 Guide가 없으면 억지 top Guide를 만들지 않는다. `B_wrong_guide_boundary`와 `D_safe_scene_overpromoted`는 0건이다.

...
def main() -> None:
    ...
    for k, gg in G.items():
        ...
        # ── 비계 공통(절1 재료·구조 + 절2의 점검) → 구체 비계 절로 상속 ──────
        # 사용자 판단 A안(2026-08-09): 앵커 오류 18건 중 9건이 "모델은 강관·이동식비계를
        # 맞게 골랐는데 감독관 인용 조문(전부 제56조 작업발판 구조)이 공통 절에만 있어
        # 흐름에 안 뜨는" 구조 문제였다(docs/dev-notes/anchor-a0-metrics-2026-08-09.md).
        # 사용 사진에 상시 성립하는 제54(재료)·제55(적재하중)·제56(발판 구조)·제58(점검·보수)만
        # 상속한다. 제57(조립·해체 절차)·별표 조립 항목은 **조립 작업 전용이라 제외**
        # (굴착기 제87~89 제외와 같은 원칙 — 사용 사진에 조립 절차는 실질 무관).
        # ★ 제56조 문언: "비계(달비계, 달대비계 및 말비계는 제외한다)" — 절4·절5는 혼합
        #   그룹이라 좌표 단위로 못 가르므로 출처 문구에 한정을 박는다(적용범위 버그 클래스 방지).
        #   걸침비계(절4)·이동식비계(절5)는 제외 목록에 없어 적용 대상이다.
        BIKE_JEOL = (1, 7)                       # 편1 총칙 > 장7 비계
        BIKE_CODES = {(1, 7, 1): ("제54조", "제55조", "제56조"),   # 절1 재료 및 구조 등
                      (1, 7, 2): ("제58조",)}                      # 절2에서 점검·보수만
        # 한정 문구 — Sol 검수(2026-08-09, bike_inherit_sol_verdicts.json 조건부 11건 수용):
        #   제54조② 는 "강관비계의 재료"에만 적용 / 제56조는 "높이 2m 이상 작업장소" 조건 /
        #   제58조 점검은 "악천후 중지 후 또는 조립·해체·변경 후" 작업 재개 시에 발동.
        BIKE_54_LIMIT = {3: "제2항(한국산업표준)은 강관비계에 한함",
                         4: "제1항만(제2항은 강관비계 전용)", 5: "제1항만(제2항은 강관비계 전용)",
                         6: "제1항만(제2항은 강관비계 전용)"}
        BIKE_56_GROUP = {4: "걸침비계에 한함(달비계·달대비계 제외)",
                         5: "이동식비계에 한함(말비계 제외)"}
        # '작업 재개 시'는 원문보다 좁다(조립·해체·변경 후 **최초 작업**도 포함) — Sol 2차 지적 수용
        BIKE_58_NOTE = "악천후 중지·조립/해체/변경 후, 그 비계 작업 시작 전"
        if (p, j) == BIKE_JEOL and jeol in (3, 4, 5, 6):
            for src_c3, codes in BIKE_CODES.items():
                src_grp = next((kk for kk, g2 in G.items()
                                if tuple(g2["coord"])[:3] == src_c3 and g2["coord"][3] is None), None)
                if not src_grp:
                    continue
                for c in codes:
                    if c not in G[src_grp]["codes"] or c in own:
                        continue
                    parts = []
                    if c == "제54조":
                        parts.append(BIKE_54_LIMIT[jeol])
                    elif c == "제56조":
                        if jeol in BIKE_56_GROUP:
                            parts.append(BIKE_56_GROUP[jeol])
                        parts.append("높이 2m 이상 작업장소")
                    elif c == "제58조":
                        parts.append(BIKE_58_NOTE)
                    lim = f" · {' · '.join(parts)}" if parts else ""
                    add_article(f"조문(비계 공통 상속{lim})", c)
                    own.add(c)

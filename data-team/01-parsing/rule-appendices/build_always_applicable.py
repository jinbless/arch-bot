...
TOPICS = [
    ...
    ("통로·계단", "통로와 계단의 구조 기준. 만들 때 지켜야 하고, 이후 그 상태를 유지해야 한다",
     ["장3 통로"]),
    # '추락·붕괴 방지'(절1·절2)는 2026-08-13 앵커 카탈로그로 승격 — 개구부·단부·절토면은
    # 사진에서 식별되는 장소 기인물이다(비계 선례. prod 실증: 개구부 사진의 흐름·즉시조치가
    # 통째로 비었다). 담는 기준 ①(카탈로그 밖)을 더는 만족하지 않아 여기서 뺀다 —
    # 남겨두면 아래 catalog 가드가 어차피 걸러내며 경고만 남긴다.

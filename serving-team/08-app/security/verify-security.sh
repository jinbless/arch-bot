...
#   prod: nginx 경로라 BASE=https://moellab.info (업로드는 /api/v1/image, 레이트리밋 IP는 X-Forwarded-For 의존)
set -uo pipefail
...
code=$(curl -s -o /dev/null -w "%{http_code}" -F "image=@$TMP/big.png;type=image/png" "$BASE/api/v1/image" 2>/dev/null || echo "ERR")
[ "$code" = "413" ] && echo "  ✅ HTTP $code" || echo "  ⚠️  HTTP $code (기대 413)"

echo "[3] 비이미지 거부 (F6, 기대 415)"
echo "this is not an image" > "$TMP/fake.png"
code=$(curl -s -o /dev/null -w "%{http_code}" -F "image=@$TMP/fake.png;type=image/png" "$BASE/api/v1/image" 2>/dev/null || echo "ERR")

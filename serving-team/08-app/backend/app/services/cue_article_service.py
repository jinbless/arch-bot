...
# RESOLVE — scripts/gimulmul_match.py:49-57 이식. **두 사본은 글자 단위 동일해야 한다**(측정=서빙 고정).
# ⚠ 프롬프트 개입 시도 2종(이탈구도 0~4개, 포괄 강등) 채택 안 함(2026-08-12 Track A-2):
#   변형 3회 재계측에서 전부 선우개발 위험물 표지 사진이 기권(빈 배열)해 exact/flow -1장이었는데,
#   **원문 프롬프트로 되돌려도 같은 기권이 재현**됐다 — 그 사진은 RESOLVE 재추첨 경계 사례고
#   (empty 0↔1이 표본에 따라 흔들림), 프롬프트 효과는 gold51 ±1장 변동과 구분 불가.
#   이득 증거 없음 + 기권 리스크 관찰 → 보수 기본값(원문) 유지. 측정 정본은 커밋된 캐시(고정 표본).
#   장소성/포괄 판정은 이 프롬프트가 아니라 judge(flow_service, H2)가 흐름 실물을 보고 내린다.
#   이 프롬프트는 계측 없이 고치지 말 것.
RESOLVE_SYS = (

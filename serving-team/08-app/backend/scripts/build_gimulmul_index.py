...
#
# ⚠ **'장6 추락 또는 붕괴'를 뺐다(2026-08-13, 사용자 결정 — 비계 선례의 재적용).**
#   절1 추락·절2 붕괴는 횡단이 아니라 **장소 기인물**이다:
#   - 개구부·슬라브 단부·계단 상부·지붕·절토면은 사진에서 형태로 식별된다
#     (anchor_validity.json도 두 그룹을 kind='장소'로 판정해 두었다)
#   - alias가 이미 절1로 배선돼 있었다(슬라브단부 121·바닥개구부 100·계단 73 — SIF/case 실증)
#   - 카탈로그에 없어서 개구부 사진의 앵커가 보건편 포괄 그룹으로 뭉갰다(prod 실증
#     2026-08-12 경북환경: FALL 1.0인데 앵커='잔재물 등의 조치기준', 추락 조문 0건 노출)
#   장2 작업장·장3 통로·장4 보호구는 진짜 횡단(시간축 없음)이라 유지 — basics 전용.
#
# ⚠ 이 목록을 바꾸면 RESOLVE 카탈로그가 바뀌므로 **앵커 정확도를 재측정해야 한다**
#   (rerun_resolve_cache.py → measure_anchor_accuracy.py). 0.711은 비계를 뺀 99종에서,
#   0.784는 106종 이전(104종)에서 잰 값이다. exact는 집합 교집합이라 '주 앵커(첫 순위)
#   절도'를 못 잡는다 — first-key diff 게이트(G-FIRST)를 함께 볼 것.
CROSS_CUTTING_JANG = [
    "장2 작업장", "장3 통로", "장4 보호구",
]
...
GIM_ENRICH = {
    ...
    # 제239~246조: 화기 사용·용접·소화설비·화재감시자. 흡연 흔적 세목은 miss11 분해 후속
    # (제니알테크 — 휴게공간의 재떨이 깡통·라이터를 화기 그룹과 연상 못 함, 2026-08-10)
    "절2 화기 등의 관리":
        "화기 작업ㆍ용접ㆍ소화기ㆍ소화설비ㆍ흡연구역ㆍ재떨이ㆍ담배꽁초ㆍ라이터 화재방지",
    # 제422~428조: 관리대상 유해물질 설비·국소배기 — 라벨이 '설비기준 등'뿐이라 실험실
    # 장면과 연상 불가(miss11 분해 — 노원병원 검사실, 안전편 위험물로 오귀속). 장면 단서
    # (실험실 시약ㆍ검사실)는 조문 제목이 아니라 분해 사진 근거임을 명시해 둔다.
    # ⚠ '국소배기장치' 토큰 금지 — law3_targets 법 제42조 이름 매칭(가장 구체적 1개 선택)이
    #   절3(국소배기장치의 성능 등)의 유해위험방지계획서 부착을 빼앗는다(flow diff 실측).
    #   동명이기 함정과 같은 클래스라 '배기 후드'로 우회한다.
    "절2 설비기준 등":
        "관리대상 유해물질 취급 설비ㆍ배기 후드ㆍ실험실 시약ㆍ유기용제ㆍ의료기관 검사실",
    # 제301~312조: 충전부·접지·누전차단기 — 절2(배선·이동전선)와의 판별 단서
    "절1 전기 기계ㆍ기구 등으로 인한 위험 방지":
        "전기 기계ㆍ기구ㆍ충전부 방호ㆍ접지ㆍ누전차단기ㆍ분전반 기기",
    # 제67~68조: 말비계·이동식비계 — 절3(강관비계)과의 형태 판별 단서(miss11 분해 + 레버 ③,
    # 2026-08-10). 슬래브 위 자립 1스팬 사다리 일체형 틀 타워는 벽면 다열 연속 강관비계와
    # 달리 이 절이다. ⚠ '고소작업대'는 별도 그룹이 있어 라벨에 넣지 않는다(충돌).
    "절5 말비계 및 이동식비계":
        "말비계ㆍ이동식비계ㆍ바퀴 달린 틀 비계ㆍ실내 자립 1스팬 사다리 일체형 틀 타워",
    # ── 보건편 무정보 라벨 4종 (Track A-1, 2026-08-12) ──────────────────
    # 이름이 사실상 같은 '설비 기준' 류 4그룹의 라벨이 절 이름 그대로(판별 정보 0)라, 장면에
    # '설비/시설' 단어만 있어도 끌려온다 — prod 실측: 계단·보행로 사진 2건이 분진 편
    # '설비 등의 기준'으로 오귀속. 각 조각은 소속 조문 제목(제607~611·560~561·453~455·
    # 499~501조)에 근거한 사실 나열이다.
    # ⚠ '국소배기장치'·'전체환기장치'·'환기장치' 토큰 금지(law3 제42조 이름 매칭 절도 —
    #   위 절2 설비기준 등 주석과 동일 함정). '배기 설비'·'환기 설비'로 우회한다.
    # ⚠ '허가대상 유해물질'을 ㆍ로 독립시키면 별표 2(관리감독자 직무) '허가대상 유해물질…작업'
    #   행과 이름 매칭돼 ASSIGN 3건이 오부착된다(2026-08-12 flow diff 실측 — 함정 4번째).
    #   공백 융합('허가대상 유해물질 석면 취급…')으로 독립 변형을 막는다.
    "절2 설비 등의 기준":
        "분진 발생 작업장ㆍ분쇄 연마 절단 공정ㆍ배기 설비ㆍ살수 습기 유지",
    "절2 설비 기준 등":
        "고열ㆍ한랭ㆍ다습 작업장 온도 습도 조절ㆍ냉난방 환기 설비",
    "절2 설비기준 및 성능 등":
        "허가대상 유해물질 석면 취급 밀폐 설비ㆍ배출액 처리",
    "절2 시설ㆍ설비기준 및 성능 등":
        "금지유해물질 취급 실험실 시설ㆍ밀폐 설비ㆍ불침투 바닥",
    # ── 장6 카탈로그 복귀 2종 (2026-08-13, 사용자 문안 확정) ─────────────
    # 절 이름 그대로('추락에 의한 위험 방지')는 판별 정보 0 — 재해 형태 어휘라 어느 사진에나
    # 걸린다. 조각은 소속 조문 제목(제43 개구부·제45 지붕·제42 추락)과 alias 실증 어휘
    # (슬라브단부·바닥개구부·계단)에 근거한 사실 나열이다.
    # ⚠ '계단'은 독립 조각 금지(장3 통로 조문 제목과 이름 매칭 — 공백 융합, 함정 4호 선례).
    # ⚠ '추락 방호망ㆍ안전대 부착설비' 조각은 기각(2026-08-13 검증) — 고소작업대·밀폐공간
    #   '안전대 등'·비계 어휘와 겹쳐 장비 앵커 절도 리스크만 늘고 장소 판별 기여가 없다.
    "절1 추락에 의한 위험 방지":
        "개구부ㆍ바닥 슬라브 개방 단부ㆍ계단 상부 개방 단부ㆍ지붕 위 작업ㆍ난간 없는 추락 위험 장소",
    # 제50 토사·제51/52 구축물 근거. ⚠ '굴착' 토큰 금지(별표4 '굴착작업' + 굴착기·노천굴착
    # 그룹과 이름 매칭 절도). '흙막이'는 노천굴착(흙막이지보공)과 실세계 중첩 — G-FIRST로 감시.
    "절2 붕괴 등에 의한 위험 방지":
        "토사 사면ㆍ흙막이 없는 수직 절토면ㆍ구축물 균열 기울어짐 붕괴 위험 장소",
}
